...
void ptzcam::rawdata2cvframe(){
    unsigned char *imgrawdata = this->getrawdata();
    memcpy(this->cv_frame.data, &imgrawdata[4],
            sizeof(unsigned char)*this->frame_height*this->frame_width*3);
}
...
void *ptzcam::test_rawdisplay(){
#ifdef PRINT_FUNC
    ENTER_FUNC
#endif
    cv::namedWindow("test_rawDisplay",1);
    this->capture_cv();
    for(;;){
        cv::imshow("test_rawDisplay",this->getcvframe());
        if(cv::waitKey(30)>=0)
            break;
    }
#ifdef PRINT_FUNC
...
void ptzcam::setrawdata(unsigned char* data){
#ifdef PRINT_FUNC
    ENTER_FUNC
#endif
    pthread_mutex_lock(&this->pkgAccessMuxid);
...
void ptzcam::setframewidth(int w){
#ifdef PRINT_FUNC
    ENTER_FUNC
#endif
    this->frame_width = w;
...
void ptzcam::setframeheight(int h){
#ifdef PRINT_FUNC
    ENTER_FUNC
#endif
    this->frame_height = h;
