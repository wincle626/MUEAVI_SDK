...
void *udp::tx2(){
    ...
    try {
        ...
        int total_pack = 1 + (int)((tosendlen - 1) / UDP_BUFFER_SIZE);

        //std::cout << tosendlen << " data,"
        //		  << total_pack << " packages to send" << std::endl;
        ...
        if(total_pack>1){
			for (i = 0; i < total_pack-1; i++){
	        	...
	        	//std::cout << i << "th sent package, "
	        	//		  << tosendlen << " data left" << std::endl;
			}
			if(tosendlen>0){
				...
				//std::cout << "tailed " << i << "th sent package" << std::endl;
			}
			/*sock.sendTo( & tosenddata[i * UDP_BUFFER_SIZE],
...
void *udp::rx2(){
    ...
    try {
        ...
        //std::cout << total_pack << " packages to receive" << std::endl;
...
void *udp::rx3(){
    ...
    std::string ipaddress = std::string(this->getRxIP()); // First arg: server address
    unsigned short portnum = (unsigned short)this->getRxPort();
    ...
    socket.bind(boost::asio::ip::udp::endpoint(boost::asio::ip::address::from_string(ipaddress), portnum));

    /*socket.receive_from(boost::asio::buffer(recv_buffer), remote_endpoint);
    io_service.run();
    int size = std::stoi(std::string(recv_buffer.elems));
    std::cout << "Received: '" << size << std::endl;*/

    int i;
    int size = this->getRxDataSize();
    int torevlen = size;
    int torevpacknum = 1 + (torevlen - 1)/4096;
	char *data_rx = NULL;
    data_rx = (char*) realloc(data_rx, size);
    for(i=0;i<torevpacknum-1;i++){
        socket.receive_from(boost::asio::buffer(recv_buffer), remote_endpoint);
        io_service.run();
        torevlen = torevlen - 4096;
        memcpy(&data_rx[i*4096],recv_buffer.elems,4096);
        //std::cout << i << "th received package" << std::endl;
    }
    socket.receive_from(boost::asio::buffer(recv_buffer), remote_endpoint);
    io_service.run();
    memcpy(&data_rx[i*4096],recv_buffer.elems,torevlen);
    //std::cout << i << "th received package" << std::endl;

    this->setRxDataW(data_rx);
...
void *udp::tx3(){
    ...
    std::string ipaddress = std::string(this->getTxIP()); // First arg: server address
    unsigned short portnum = (unsigned short)this->getTxPort();

    boost::asio::io_service io_service;
    boost::asio::ip::udp::socket socket(io_service);
    boost::asio::ip::udp::endpoint remote_endpoint =
    		boost::asio::ip::udp::endpoint(boost::asio::ip::address::from_string(ipaddress), portnum);
    socket.open(boost::asio::ip::udp::v4());
    boost::system::error_code err;

    int size = this->getTxDataSize();
    //std::string dsize = std::to_string(size);
    //auto sent = socket.send_to(boost::asio::buffer(dsize.c_str(), dsize.size()), remote_endpoint, 0, err);

    int i;
    auto sent = 0;
    int tosentlen = size;
    int tosentpacknum = 1 + (tosentlen - 1)/4096;
    char *data_tx = this->getTxData();
    for(i=0;i<tosentpacknum-1;i++){
	    sent += socket.send_to(boost::asio::buffer(&data_tx[i*4096], 4096), remote_endpoint, 0, err);
	    tosentlen = tosentlen - 4096;
        //std::cout << i << "th sent package" << std::endl;
    }
    sent += socket.send_to(boost::asio::buffer(&data_tx[i*4096], tosentlen), remote_endpoint, 0, err);
    //std::cout << i << "th sent package" << std::endl;
